      ...
      if (b->type & JSON_PATH_ARRAY)
      {
        ...
        if (array_sizes)
        {
          ...
          corrected_n_item_a= a->n_item < 0 ?
                                *((int*)mem_root_dynamic_array_get_val(array_sizes, b-temp_b)) + a->n_item :
                                a->n_item;
        }
        if (a->type & JSON_PATH_ARRAY_RANGE)
        {
          ...
          if (array_sizes)
          {
            corrected_n_item_end_a= a->n_item_end < 0 ?
                                    *((int*)mem_root_dynamic_array_get_val(array_sizes, b-temp_b)) + (a->n_item_end) :
                                    a->n_item_end;
          }
...
longlong Item_func_json_valid::val_int()
{
  ...
  if ((json_valid(&current_mem_root, js->ptr(), js->length(), js->charset(), &je) == 0))
  {
    return 1;
  }
  else
...
bool Item_func_json_value::fix_length_and_dec(THD *thd)
{
  ...
  mem_root_dynamic_array_init(&current_mem_root, PSI_NOT_INSTRUMENTED,
                                &json_depth_array,
                                 sizeof(int), NULL,
                                 32, 32, MYF(0));
  Json_path_extractor::init_json_engine_stack(&current_mem_root, &json_engine_scan_stack);
...
void Json_path_extractor::init_json_engine_stack(MEM_ROOT *mem_root, MEM_ROOT_DYNAMIC_ARRAY *json_engine_scan_stack)
{
                                  ...
                                  String *func_name_str)
{
  ...
  je.stack= json_engine_scan_stack;
  ...
  while (json_get_path_next(&je, &p) == 0)
  {
    ...
    if (has_negative_path && je.value_type == JSON_VALUE_ARRAY &&
        json_skip_array_and_count(&je,
                                  (int*)mem_root_dynamic_array_get_val(&json_depth_array, p.last_step - psteps)))
      goto error;
  ...
  case JSON_VALUE_OBJECT:
  {
    ...
    while (json_scan_next(value) == 0 && value->state != JST_OBJ_END)
    {
      ...
      if (!find_key_in_object(js, &key_name) ||
          json_read_value(js) ||
          !check_contains(mem_root, js, value, loc_js_stack))
        return FALSE;
    }
  ...
  case JSON_VALUE_ARRAY:
    if (value->value_type != JSON_VALUE_ARRAY)
    {
      ...
      while (json_scan_next(js) == 0 && js->state != JST_ARRAY_END)
      {
        ...
        if (check_contains(mem_root, js, value, loc_js_stack))
        {
...
bool Item_func_json_object_filter_keys::fix_length_and_dec(THD *thd)
{
  ...
  if (json_read_value(&je2) || je2.value_type != JSON_VALUE_ARRAY ||
      create_hash(&je2, &items, hash_inited, &hash_root, &current_mem_root))
  {
    if (je2.s.error)
