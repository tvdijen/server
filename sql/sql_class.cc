                                 ...
                                 myf my_flags)
{
  ...
  */
  if (init_alloc &&
      !(array->buffer= (uchar*) alloc_root(current_mem_root? current_mem_root: current_thd->mem_root, init_alloc)))
    array->max_element=0;
  DBUG_RETURN(FALSE);
}
...
int mem_root_dynamic_array_set_val(MEM_ROOT_DYNAMIC_ARRAY *array, const void *element, size_t idx)
{
  ...
  {
    if (idx >= array->max_element && mem_root_allocate_dynamic(mem_root, array, idx))
      return TRUE;
