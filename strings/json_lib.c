...
static int skip_str_constant(json_engine_t *j)
{
  ...
  j->state= *((int*)mem_root_dynamic_array_get_val(&(j->stack) ,j->stack_p));
  return 0;
}
                        ...
                        MEM_ROOT_DYNAMIC_ARRAY *array_counters)
{
  ...
  mem_root_dynamic_array_set_val(array_counters, 0, (next_step - psteps));
                   ...
                   MEM_ROOT_DYNAMIC_ARRAY *array_counters)
{
  ...
  do
  {
    ...
    case JST_VALUE:
      ...
      {
        res= (cur_step->n_item <= *((int*)mem_root_dynamic_array_get_val(array_counters, cur_step - psteps)) &&
              cur_step->n_item_end >= *((int*)mem_root_dynamic_array_get_val(array_counters, cur_step - psteps)));
        value= *((int*)mem_root_dynamic_array_get_val(array_counters, cur_step - psteps))+1;
    ...
    case JST_OBJ_END:
      ...
      do
      {
        (*p_cur_step)--;
                ...
                mem_root_dynamic_array_set_val(array_counters, &value, *p_cur_step-psteps));
      break;
    case JST_ARRAY_END:
      (*p_cur_step)--;
      break;
...
{
  uint p_c;
  int path_found, no_match_found, val;
    ...
    case JST_VALUE:
      ...
      for (p_c=0; p_c < state->n_paths; p_c++)
      {
        ...
        if (state->path_depths[p_c]< state->cur_depth /* Path already failed. */ ||
          ...
          continue;
        if (cur_step->type & JSON_PATH_WILD ||
            cur_step->n_item == *(int*)mem_root_dynamic_array_get_val(&state->array_counters, state->cur_depth))
        {
          /* Array item matches. */
      ...
      val= *(int*)mem_root_dynamic_array_get_val(&state->array_counters, state->cur_depth)+1;
...
int json_get_path_next(json_engine_t *je, json_path_t *p)
{
  ...
  if (p->last_step < (json_path_step_t*)(p->steps.buffer))
  {
    ...
    p->last_step= (initial_step + (p->steps.elements));
...
*/
int json_valid(MEM_ROOT *mem_root, const char *js, size_t js_len, CHARSET_INFO *cs, json_engine_t *temp_je)
{
  json_engine_t je;
  ...
  if (!mem_root)
  {
    needs_local_mem_root= 1;
    init_alloc_root(PSI_NOT_INSTRUMENTED, &new_mem_root, 1024, 0, MYF(0));
  }

  mem_root_dynamic_array_init(mem_root ? mem_root:&new_mem_root, PSI_NOT_INSTRUMENTED,
                                &je.stack,
                                 sizeof(int), NULL,
                                 32, 32, MYF(0));
